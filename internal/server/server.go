...
func (app *App) searchDocsHandler(_ context.Context, _ *mcp.CallToolRequest, args searchDocsArgs) (*mcp.CallToolResult, any, error) {
	...
	resp, err := app.DB.SearchDocs(db.SearchParams{
		Query:    args.Query,
		Mode:     args.Mode,
	...
	return toolJSON("search_docs", resp), nil, nil
}

type applyModernizeArgs struct {
	ProjectPath string   `json:"project_path"`
	Fixers      []string `json:"fixers,omitempty"`
	Aggressive  bool     `json:"aggressive,omitempty"`
	DryRun      bool     `json:"dry_run,omitempty"`
}

func (app *App) applyModernizeHandler(ctx context.Context, _ *mcp.CallToolRequest, args applyModernizeArgs) (*mcp.CallToolResult, any, error) {
	if strings.TrimSpace(args.ProjectPath) == "" {
		return toolError("project_path is required"), nil, nil
	}
	absPath, err := filepath.Abs(args.ProjectPath)
	...
	diags, err := diagnostics.ApplyModernize(ctx, absPath, args.Fixers, args.Aggressive, args.DryRun)
	if err != nil {
		return toolError("apply_modernize failed: %v", err), nil, nil
	}
	msg := "apply_modernize"
	if args.DryRun {
		msg = "apply_modernize (dry-run)"
	}
	return toolJSON(msg, diags), nil, nil
}
